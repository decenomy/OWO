...
pkgdatadir = $(datadir)/OneWorld
pkgincludedir = $(includedir)/OneWorld
pkglibdir = $(libdir)/OneWorld
pkglibexecdir = $(libexecdir)/OneWorld
...
CONFIG_HEADER = $(top_builddir)/src/config/OneWorld-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/OneWorld-config.h.in \
...
ACLOCAL = ${SHELL} /root/OneWorld_linux/OneWorld/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /root/OneWorld_linux/OneWorld/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/OneWorld_linux/OneWorld/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/OneWorld_linux/OneWorld/build-aux/missing automake-1.15
AVX2_CXXFLAGS = -mavx -mavx2
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = OneWorld-cli
BITCOIN_DAEMON_NAME = OneWorldd
BITCOIN_GUI_NAME = OneWorld-qt
BITCOIN_TX_NAME = OneWorld-tx
...
CPPFLAGS = -I/root/OneWorld_linux/OneWorld/db4/include/ -DHAVE_BUILD_INFO -D__STDC_FORMAT_MACROS
CRYPTO_CFLAGS = 
CRYPTO_LIBS = -lcrypto
CXX = g++ -std=c++11
CXXCPP = g++ -std=c++11 -E
...
LDFLAGS = -L/root/OneWorld_linux/OneWorld/db4/lib/
LEVELDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /root/OneWorld_linux/OneWorld/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = OneWorld
PACKAGE_BUGREPORT = www.OneWorld.org
PACKAGE_NAME = OneWorld Core
PACKAGE_STRING = OneWorld Core 3.0.0
PACKAGE_TARNAME = OneWorld
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /root/OneWorld_linux/OneWorld
abs_srcdir = /root/OneWorld_linux/OneWorld
abs_top_builddir = /root/OneWorld_linux/OneWorld
abs_top_srcdir = /root/OneWorld_linux/OneWorld
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/OneWorld_linux/OneWorld/build-aux/install-sh
...
pdfdir = ${docdir}
prefix = /root/OneWorld_linux/OneWorld/depends/x86_64-pc-linux-gnu
...
DIST_CONTRIB = $(top_srcdir)/contrib/OneWorld-cli.bash-completion \
	       $(top_srcdir)/contrib/OneWorld-tx.bash-completion \
	       $(top_srcdir)/contrib/OneWorldd.bash-completion \
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/OneWorld.ico \
  $(top_srcdir)/share/pixmaps/nsis-header.bmp \
  $(top_srcdir)/share/pixmaps/nsis-wizard.bmp \
  $(top_srcdir)/doc/README_windows.txt

LINUX_PACKAGING = $(top_srcdir)/share/pixmaps/OneWorld16.xpm \
  $(top_srcdir)/share/pixmaps/OneWorld32.xpm \
  $(top_srcdir)/share/pixmaps/OneWorld128.png
...
COVERAGE_INFO = baseline.info \
  test_OneWorld_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_OneWorld_coverage.info test_OneWorld.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/OneWorld-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/OneWorld-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/OneWorld-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/OneWorld-config.h
$(top_srcdir)/src/config/OneWorld-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/OneWorld-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/OneWorld-Qt: all-recursive
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/OneWorld-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/OneWorld-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_OneWorld.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_OneWorld -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_OneWorld_filtered.info: test_OneWorld.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_OneWorld_filtered.info
...
#test_OneWorld_coverage.info: baseline_filtered.info test_OneWorld_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_OneWorld_filtered.info -o $@

#total_coverage.info: test_OneWorld_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_OneWorld_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_OneWorld.coverage/.dirstamp:  test_OneWorld_coverage.info
...
#cov: test_OneWorld.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_OneWorld.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
