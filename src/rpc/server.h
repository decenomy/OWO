...
/**
 * OneWorld RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZOWOSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzOWOseed(const UniValue& params, bool fHelp);
extern UniValue getzOWOseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzOWO(const UniValue& params, bool fHelp);
extern UniValue dzOWOstate(const UniValue& params, bool fHelp);
