...
CAmount CMasternode::GetMasternodePayment(int nHeight)
{
    ...
    if (nHeight > 900000) return blockValue * 65 / 100;
    if (nHeight > 800000) return blockValue * 70 / 100;
    if (nHeight > 700000) return blockValue * 75 / 100;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 OWO tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                   // block for 1000 OWO tx -> 1 confirmation
