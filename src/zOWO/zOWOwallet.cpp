// Copyright (c) 2017-2019 The OneWorld developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zOWOwallet.h"
#include "main.h"
#include "txdb.h"
...
CzOWOWallet::CzOWOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zOWO seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZOWOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZOWOSeed_deprecated()) {
                    LogPrintf("%s: Updated zOWO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zOWO seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zOWO wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzOWOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZOWOCount(nCountLastUsed);
    else if (!walletdb.ReadZOWOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzOWOWallet::Lock()
{
    seedMaster = 0;
}

void CzOWOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzOWOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZOWO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzOWOWallet::LoadMintPoolFromDB()
{
...
void CzOWOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzOWOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzOWOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zOWOTracker->HasPubcoinHash(pMint.first)) {
...
bool CzOWOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZOWO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zOWOTracker which also adds to database
    pwalletMain->zOWOTracker->Add(dMint, true);
...
void CzOWOWallet::SeedToZOWO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzOWOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzOWOWallet::GenerateDeterministicZOWO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzOWOWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzOWOWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzOWOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
