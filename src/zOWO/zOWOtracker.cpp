// Copyright (c) 2018-2019 The OneWorld developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zOWO/deterministicmint.h>
#include "zOWOtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzOWOTracker::CzOWOTracker(std::string strWalletFile)
{
...
CzOWOTracker::~CzOWOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzOWOTracker::Init()
{
...
bool CzOWOTracker::Archive(CMintMeta& meta)
{
...
bool CzOWOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzOWOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzOWOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzOWOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzOWOTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzOWOTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzOWOTracker::GetSerialHashes()
{
...
CAmount CzOWOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzOWOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzOWOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzOWOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzOWOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzOWOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzOWOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzOWOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzOWOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzOWOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzOWOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzOWOWallet* zOWOWallet)
{
    bool iszOWOWalletInitialized = (NULL != zOWOWallet);
    CMintMeta meta;
    ...
    if (! iszOWOWalletInitialized)
        zOWOWallet = new CzOWOWallet(strWalletFile);
    meta.isSeedCorrect = zOWOWallet->CheckSeed(dMint);
    if (! iszOWOWalletInitialized)
        delete zOWOWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzOWOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzOWOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzOWOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzOWOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzOWOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzOWOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzOWOWallet* zOWOWallet = new CzOWOWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zOWOWallet);
        }
        delete zOWOWallet;
        LogPrint("zero", "%s: added %d dzOWO from DB\n", __func__, listDeterministicDB.size());
    }
