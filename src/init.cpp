...
#include "zOWO/accumulatorcheckpoints.h"
#include "zOWOchain.h"
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZOWOThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the OWO and zOWO money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 32112, 51474));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in OWO/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in OWO/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    std::string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, OneWorld, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in OWO/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-OWOstake=<n>", strprintf(_("Enable or disable staking functionality for OWO inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zOWOstake=<n>", strprintf(_("Enable or disable staking functionality for zOWO inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzOWO=<n>", strprintf(_("Enable automatic wallet backups triggered after each zOWO minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zOWO spends and stakes (0-1, default %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-zOWObackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zOWO backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 32113, 51475));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The OneWorld Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //OneWorld: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //OneWorld specific: zerocoin and spork DB's
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    CAmount zOWOSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();

                    if (pblockindex->GetZerocoinSupply() < zOWOSupplyCheckpoint) {
                        // Trigger reindex due wrapping serials
                        LogPrintf("Current GetZerocoinSupply: %d vs %d\n", pblockindex->GetZerocoinSupply()/COIN , zOWOSupplyCheckpoint/COIN);
                        reindexDueWrappedSerials = true;
                    } else if (pblockindex->GetZerocoinSupply() > zOWOSupplyCheckpoint) {
                        // Trigger global zOWO reindex
                        reindexZerocoin = true;
                        LogPrintf("Current GetZerocoinSupplyssss: %d vs %d\n", pblockindex->GetZerocoinSupply()/COIN , zOWOSupplyCheckpoint/COIN);
                    }
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    if (chainHeight > Params().Zerocoin_StartHeight()) {
                        RecalculateZOWOMinted();
                        RecalculateZOWOSpent();
                    }
                    // Recalculate from the zerocoin activation or from scratch.
                    RecalculateOWOSupply(reindexZerocoin ? Params().Zerocoin_StartHeight() : 1);
                }
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    CAmount zOWOSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();
                    if (pblockindex->GetZerocoinSupply() != zOWOSupplyCheckpoint)
                        return InitError(strprintf("ZerocoinSupply Recalculation failed: %d vs %d", pblockindex->GetZerocoinSupply()/COIN , zOWOSupplyCheckpoint/COIN));
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzOWOWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zOWOWallet
        uiInterface.InitMessage(_("Syncing zOWO wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZOWOBackups = GetBoolArg("-backupzOWO", true);
        pwalletMain->setZOWOAutoBackups(fEnableZOWOBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zOWOTracker->Init();
    ...
    /* Denominations
       ...
       For example:
       1OWO+1000 == (.1OWO+100)*10
       10OWO+10000 == (1OWO+1000)*10
    */
    obfuScationDenominations.push_back((500000 * COIN) + 10000000);
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZOWOThread(threadGroup);
    }
    ...
    if (pwalletMain) {
        ...
        if (GetBoolArg("-precompute", false)) {
            // Run a thread to precompute any zOWO spends
