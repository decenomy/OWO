...
#include "ui_zOWOcontroldialog.h"
...
std::set<std::string> ZOneWorldControlDialog::setSelectedMints;
std::set<CMintMeta> ZOneWorldControlDialog::setMints;

bool CZOneWorldControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZOneWorldControlDialog::COLUMN_DENOMINATION || column == ZOneWorldControlDialog::COLUMN_VERSION || column == ZOneWorldControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZOneWorldControlDialog::ZOneWorldControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZOneWorldControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZOneWorldControlDialog::~ZOneWorldControlDialog()
{
    delete ui;
}

void ZOneWorldControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOneWorldControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZOneWorldControlWidgetItem* itemDenom(new CZOneWorldControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZOneWorldControlWidgetItem *itemMint = new CZOneWorldControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...

        {
            ...
            LOCK(pwalletMain->zOWOTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zOWOTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zOneWorld.";
            else if (!mint.isSeedCorrect)
                strReason = "The zOneWorld seed used to mint this zOneWorld is not the same as currently hold in the wallet";
            else
...
void ZOneWorldControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOneWorldControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZOneWorld_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOneWorldControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZOneWorldControlDialog::GetSelectedMints()
{
...
void ZOneWorldControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
