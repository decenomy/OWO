...
class QtRPCTimerBase: public QObject, public RPCTimerBase
{
    Q_OBJECT
public:
    QtRPCTimerBase(boost::function<void(void)>& OneWorld, int64_t millis):
        OneWorld(OneWorld)
    {
    ...
    ~QtRPCTimerBase() {}
private slots:
    void timeout() { OneWorld(); }
private:
    QTimer timer;
    boost::function<void(void)> OneWorld;
};
...
public:
    ...
    RPCTimerBase* NewTimer(boost::function<void(void)>& OneWorld, int64_t millis)
    {
        return new QtRPCTimerBase(OneWorld, millis);
    }
};
                                          ...
                                          banTableContextMenu(0)
{
    ...
    std::string strzOneWorldPathCustom = GetArg("-OWObackuppath", "");
    ...
    if(!strzOneWorldPathCustom.empty()) {
        ui->wallet_customOWObackuppath->setText(QString::fromStdString(strzOneWorldPathCustom));
        ui->wallet_customOWObackuppath_label->setVisible(true);
        ui->wallet_customOWObackuppath->setVisible(true);
    }

    if((!strPathCustom.empty() || !strzOneWorldPathCustom.empty()) && nCustomBackupThreshold > 0) {
