...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("OneWorld client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("OneWorld"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("OneWorld"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
