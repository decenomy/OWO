...
// Copyright (c) 2019-2020 The OWO developers
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    int nDevIndex = wtx.vout.size() - 1;

    if (wtx.HasZerocoinSpendInputs()) {
 //       libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZOneWorld::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
   //     fZSpendFromMe = wallet->IsMyZerocoinSpend(zcspend.getCoinSerialNumber());
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zOWOTracker->HasMintTx(hash))) {
            //zOWO stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZOneWorld;
            ...
            sub.credit = nNet;
        } else   if (isminetype mine = wallet->IsMine(wtx.vout[nDevIndex])) {
            // OWO dev reward
            if (ExtractDestination(wtx.vout[nDevIndex].scriptPubKey, address) && IsMine(*wallet, address)) {
            sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
...
bool IsZOneWorldType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZOneWorld:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZOneWorld || type == TransactionRecord::MNReward) {
