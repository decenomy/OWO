...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 OneWorld.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzOWOReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzOWOReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzOWOReset.connect(boost::bind(NotifyzOWOReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzOWOReset.disconnect(boost::bind(NotifyzOWOReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zOWOTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
