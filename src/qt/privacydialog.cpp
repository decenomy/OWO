                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zOneWorld ought to be enough for anybody." - Bill Gates, 2017
    ui->zOneWorldpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzOneWorldSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOneWorldpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzOneWorld_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOneWorld is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zOneWorld, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOneWorld...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOneWorld in ") +
...
void PrivacyDialog::on_pushButtonSpendzOneWorld_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOneWorld is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zOneWorld, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zOneWorld
        sendzOneWorld();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOneWorld
    sendzOneWorld();
}
...
void PrivacyDialog::sendzOneWorld()
{
    QSettings settings;
    ...
    double dAmount = ui->zOneWorldpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOneWorld is requested
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOneWorld</b>";
    ...
    if (!ZOneWorldControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZOneWorldControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zOneWorld transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zOneWorld was spent successfully update the addressbook with the label
    ...
    // Clear OWO selector in case it was used
    ZOneWorldControlDialog::setSelectedMints.clear();
    ui->labelzOneWorldSelected_int->setText(QString("0"));
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zOneWorldpayAmount->setText ("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zOWOTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOneWorld </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOneWorld "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOneWorld "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zOneWorld "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zOneWorld </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zOneWorld </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zOneWorld </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzOneWorldSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzOneWorld->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zOneWorld
        //ui->pushButtonMintzOneWorld->setEnabled(false);
        //ui->pushButtonMintzOneWorld->setToolTip(tr("zOneWorld is currently disabled due to maintenance."));

        // Spend zOneWorld
        ui->pushButtonSpendzOneWorld->setEnabled(false);
        ui->pushButtonSpendzOneWorld->setToolTip(tr("zOneWorld is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zOneWorld
        //ui->pushButtonMintzOneWorld->setEnabled(true);
        //ui->pushButtonMintzOneWorld->setToolTip(tr("PrivacyDialog", "Enter an amount of OneWorld to convert to zOneWorld", 0));

        // Spend zOneWorld
        ui->pushButtonSpendzOneWorld->setEnabled(true);
        ui->pushButtonSpendzOneWorld->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
