...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zOWOspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 OWO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zOWO send
enum ZerocoinSpendStatus {
    ZOWO_SPEND_OKAY = 0,                            // No error
    ZOWO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZOWO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZOWO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZOWO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZOWO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZOWO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZOWO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZOWO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZOWO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZOWO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZOWO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZOWO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZOWO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZOWO_SPENT_USED_ZOWO = 14,                      // Coin has already been spend
    ZOWO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZOWO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZOWOBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZOWOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzOWOAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzOWOTracker> zOWOTracker;
    ...
    void setZWallet(CzOWOWallet* zwallet)
    {
        zwalletMain = zwallet;
        zOWOTracker = std::unique_ptr<CzOWOTracker>(new CzOWOTracker(strWalletFile));
    }

    CzOWOWallet* getZWallet() { return zwalletMain; }
    ...
    void setZOWOAutoBackups(bool fEnabled)
    {
    ...
    /** zOWO reset */
    boost::signals2::signal<void()> NotifyzOWOReset;
