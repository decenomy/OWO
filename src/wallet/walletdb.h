...
// Copyright (c) 2016-2019 The OneWorld developers
...
public:
    ...
    bool WriteZOWOSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZOWOSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZOWOSeed_deprecated(uint256& seed);
    bool EraseZOWOSeed();
    bool EraseZOWOSeed_deprecated();

    bool WriteZOWOCount(const uint32_t& nCount);
    bool ReadZOWOCount(uint32_t& nCount);
